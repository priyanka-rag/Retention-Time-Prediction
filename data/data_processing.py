'''Author: Priyanka Raghavan
   Description: data read-in, pre-processing, featurization, and random splits'''
...
def split_data(inchi_list,smiles_list,retention_time_list,toSave): #generates holdout test set and sends to a csv file, also sends remaining test/validation set to csv file
  ...
  if toSave: #if we want to send to csv file
    df_train.to_csv('/content/drive/My Drive/10.C51 Project/train_valid_set.csv')
    df_test.to_csv('/content/drive/My Drive/10.C51 Project/holdout_test_set.csv')

  return df_train,df_test

def read_randomsplit_data():
    df_train = pd.read_csv(path + '/data/train_valid_set.csv')
    df_test = pd.read_csv(path + '/data/holdout_test_set.csv')
    smiles_train = df_train['SMILES'].tolist()
    fgp_train = np.array(genFingerprints(smiles_train))
    rt_train =  df_train['RT'].tolist()
    smiles_test = df_test['SMILES'].tolist()
    fgp_test = np.array(genFingerprints(smiles_test))
    rt_test = df_test['RT'].tolist()
    return smiles_train,smiles_test,fgp_train,fgp_test,rt_train,rt_test